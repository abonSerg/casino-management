...
the **GammaStack "ARC"** sandbox (back office + player site) plus market and technical research.
...
4. **The licensing fork decides more than the price.** White label means operating on the vendor's licence.
...
Investigation was **read-only**. Forms were opened to read their fields; nothing was submitted, no deposits or
bets were placed, and no data was modified in the vendor's sandbox. Game launch was exercised in DEMO mode only.
No direct API calls were made with the admin session token — see `NEXT-STEPS.md` §4, which is an open decision.
